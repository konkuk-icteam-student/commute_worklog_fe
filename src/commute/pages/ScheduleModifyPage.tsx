...
export default function ScheduleModifyPage() {
  ...
  const handleCancelRequest = () => {
    ...
    if (pendingRequestIndex !== -1) {
      ...
      setModificationRequests((prev) => prev.filter((_, index) => index !== pendingRequestIndex));
    }
  ...
  return (
    <div className="relative min-h-screen w-full bg-white" data-name="scheduleModify">
      {/* Background Gradient */}
      <div
      ...
      {/* Header */}
      <div
        className="w-full bg-[#51a8ff] shadow-[0px_4px_6px_-4px_rgba(0,0,0,0.1)]"
        data-name="Container"
      >
      ...
      <div className="w-full overflow-y-auto pb-[10rem]">
        <div className="mx-auto flex max-w-[39.3rem] flex-col gap-[1.2rem] px-[2.4rem] pt-[2rem]">
          {/* Info Alert */}
          <div
            className="flex w-full items-center gap-[0.8rem] rounded-[1rem] border border-[rgba(81,168,255,0.2)] bg-[rgba(81,168,255,0.05)] px-[1.2rem] py-[0.8rem]"
            data-name="Container"
          >
            <svg className="size-[1.4rem] shrink-0" fill="none" viewBox="0 0 14 14">
              <path
                d={svgPathsModify.p2ee58b00}
                stroke="#51A8FF"
                strokeLinecap="round"
                strokeLinejoin="round"
                strokeWidth="1.16646"
            ...
            {(['all', 'pending', 'approved', 'rejected'] as const).map((tab) => {
              ...
              return (
                <button
                  ...
                  className={`h-[3.6rem] rounded-[4.4rem] px-[1.6rem] transition-all duration-200 ${
                    isActive
                ...
                >
                  <p
                    className={`font-['LINE_Seed_Sans_KR:Regular',sans-serif] text-[1.3rem] leading-[1.2rem] tracking-[0.021rem] ${
          ...
          {filteredRequests.map((request, index) => {
            ...
            return (
              <div
                key={index}
                className="w-full rounded-[1.6rem] bg-white p-[2rem] shadow-[0px_4px_20px_0px_rgba(81,168,255,0.07)]"
              >
                ...
                <div className="flex flex-col gap-[0.8rem]">
                  {/* Status Badge */}
                  <div
                    className={`${colors.bg} border ${colors.border} rounded-[1rem] px-[1.2rem] py-[0.8rem]`}
                  >
                    <p
                      className={`font-['LINE_Seed_Sans_KR:Bold',sans-serif] text-[1.1rem] leading-[1.65rem] ${colors.titleColor}`}
                    >
                      {request.statusLabel}
                    </p>
                    <p
                      className={`font-['LINE_Seed_Sans_KR:Regular',sans-serif] text-[1rem] leading-[1.5rem] ${colors.descColor}`}
                    >
                  ...
                  {/* Application Date */}
                  <div className="rounded-[1rem] bg-[#f9fafb] px-[1.2rem] py-[0.8rem]">
          ...
          <button
            ...
            className="flex h-[5.6rem] w-full items-center justify-center rounded-[4.6rem] bg-[#51a8ff] shadow-[0px_4px_20px_0px_rgba(81,168,255,0.07)]"
            data-name="Button"
          >
      ...
      {showCancelModal && (
        <div
          className="fixed inset-0 z-50 flex items-center justify-center bg-[rgba(9,18,28,0.4)] backdrop-blur-[2px]"
          data-name="pop-up"
        >
          <div className="flex w-[31.2rem] flex-col items-center gap-[2.4rem]">
            {/* Confirmation Message */}
            <p className="text-center font-['LINE_Seed_Sans_KR:Bold',sans-serif] text-[1.8rem] leading-[2.8rem] tracking-[0.024rem] text-white">
              수정 요청을 취소하시겠습니까?
            </p>
            ...
            <div className="flex w-full gap-[1.2rem]">
              <button
                ...
                className="flex h-[5.2rem] flex-1 items-center justify-center rounded-[4.6rem] bg-white shadow-[0px_4px_20px_0px_rgba(81,168,255,0.07)]"
                data-name="Button"
              >
                <p className="font-['LINE_Seed_Sans_KR:Bold',sans-serif] text-[1.6rem] leading-[2.4rem] tracking-[0.024rem] text-[#09121c]">
                  닫기
                </p>
              </button>
              <button
                onClick={handleCancelRequest}
                className="flex h-[5.2rem] flex-1 items-center justify-center rounded-[4.6rem] bg-[#51a8ff] shadow-[0px_4px_20px_0px_rgba(81,168,255,0.07)]"
                data-name="Button"
              >
